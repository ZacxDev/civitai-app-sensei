{
  description = "Civitai Sensei — a Civitai App Block. Dev shell: node + pnpm, pinned.";
  ...
  outputs = { self, nixpkgs }:
    let
      # A public OSS reference block: contributors are not all on this
      # workstation, and a devShell costs nothing to *evaluate* for a system
      # nobody builds on. Only x86_64-linux is exercised in CI and locally —
      # see CLAUDE.md.
      #
      ...
      # (`block.manifest.json`'s `buildCommand: pnpm run build` runs against
      # this same package.json), a blast radius this repo's dev ergonomics do
      # not justify. So the pnpm major is stated in both places and
      # `src/toolchain-lockstep.test.ts` asserts the two agree — a red test
      ...
      devShells = forAllSystems (pkgs:
        let tc = toolchain pkgs; in
        {
          default = pkgs.mkShell {
            ...
            shellHook = ''
              echo "sensei: node $(node --version), pnpm $(pnpm --version)"
              echo "  pnpm install --frozen-lockfile   restore node_modules"
              echo "  pnpm run dev:harness             mock host on :5189"
