 ...
 * whatever node their machine happened to have. The literal is the part that
 * matters here — a version written down in one place and nowhere else looks
 * pinned while being pinned to nothing, and the failure it produces is not a
 * red X but a green run against a toolchain nobody else has.
 *
 ...
 * adding one to read a 45-line workflow would be a bigger change than the
...
describe('toolchain lockstep', () => {
  ...
  it('pins the same pnpm major in flake.nix and in CI', () => {
    ...
    // would rot on a routine `nix flake update` and turn trunk red for nothing —
